...
public class ql3 {
    ...
    private int cperf = 1; // performance penalty
    private int cres = 1; // resource cost
    ...
    public ArrayList<Double> qvalue = new ArrayList<>();
    ...
    void print() {
        FileWriter fw;
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_qtable.txt";
    ...
    void print_state_action(String action) {
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_state_action.txt";
    ...
    //         //calculate average cpu use
    //         avg /= replicas;
    //         int cpus = s.cpus + 1;
    //         cpus *= 10;
    //         avg /= cpus;
            
    //         // //average with warmup. warmup 0.1 avg  0.9
    //         // double warm = warmup();
    //         // System.out.println("avg " +avg + "warm " +warm);
    //         // avg = (0.9 * avg + 0.1 * warm);
    ...
    //calculate future cpu utilization if cpu use >80% return 1
    public int cal_use(State st ,State ne){
        double cpu = (st.use+1) * (st.cons+1) * (st.cpus+1);
        double reso =(ne.cons +1) * (ne.cpus +1);
        double use = cpu / reso;
        if(use > 8)
            return 1;
        else
...
/////////////////////////////////////////////+r
    ...
    double cost(State old, State ne) {
        ...
        double tmax = 25;
        if(con_name == "app_mnae1" || con_name == "app_mnae2")
            tmax = 5;
        if(con_name == "app_mn2")
            tmax = 15;
    ...
    void container_update(State old, State ne) {
        ...
        // update replicas
        if (ne.cons != old.cons) {
            // add flag = 0 means last action isnot +1.
            //So it can change replicas
            //Otherwise,it cannot change replicas
            if (ne.cons > old.cons) { // +1
        ...
        // Wait(20000);//wait 30s until replicas ready
        // if(con_name == "app_mn1")
        //     mod.get_cons();
    }
    ...
    void print_res_time2(double elapsed) {
        try {
    ...
    void print_share(State st) {
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_cpus.txt";
            FileWriter fw = new FileWriter(filename, true);
            fw.write(st.cpus + "\n");
            fw.flush();
            fw.close();
    ...
    void print_tmax_vio() {
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_tmax_vio.txt";
            FileWriter fw1 = new FileWriter(filename, true);
            fw1.write(tmax_vio + "\n");
            fw1.flush();
            fw1.close();
    ...
    void print_qvalue(double qvalue){
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_qvalue.txt";
            FileWriter fw1 = new FileWriter(filename, true);
            fw1.write(qvalue + "\n");
            fw1.flush();
            fw1.close();
    ...
    void print_res_cost(double res){
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_res_cost.txt";
            FileWriter fw1 = new FileWriter(filename, true);
            fw1.write(res + "\n");
            fw1.flush();
            fw1.close();
    ...
    void print_perf_cost(double perf){
        try {
            String filename = "ql3/" + con_name + "/" + con_name + "_perf_cost.txt";
            FileWriter fw1 = new FileWriter(filename, true);
            fw1.write(perf + "\n");
            fw1.flush();
            fw1.close();
    ...
    void print_avg_qvalue(double qvalue){
        try {
    ...
    public double read(String con_name){
        String filename = "ql3/" + con_name + "/" + con_name + "_qvalue2.txt";
        // System.out.println(filename);
        int numRead = 1;//read last 3 values
        int count = 0;
        double avg = 0;
        try{
